...
def get_args():
    ...
    parser.add_argument('--increment',type=float,default=0.2)
...
class Trainer(object):
    ...
    def save_checkpoint(self, epoch):
        save_state = {
            ...
            'epoch_loss_prev':self.track_loss[self.epoch-1],
            'threshold_info':self.hd_classwise_global_thresh
        }
    ...
    def train(self,epoch):
        ...
        for key in classes:
            wandb.log({'{}_hd_threshold'.format(str(key)):self.hd_classwise_global_thresh[key],'epoch':self.epoch})
            wandb.log({'Num_inst_{}'.format(str(key)):self.track_classwise_instances[str(key)],'epoch':self.epoch})

        for key in classes:
            freq=[self.track_classwise_instances[str(key)]]
            table=wandb.Table(data=freq,columns=['freq'.format(key)])
            wandb.log({'{}_instances_per_epoch'.format(str(key)):wandb.plot.histogram(table,"freq",title='Instances_per_epoch_classwise')})
