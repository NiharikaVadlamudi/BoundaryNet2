...
from models.MCNN.fullMCNN  import Model
...
def create_folder(args):
    try : 
        ...
        wandb.init(project="MCNN Experiments",id=args.wid,resume='allow')
...
def get_args():
    ...
    parser.add_argument('--wid',type=str,default=wandb.util.generate_id())
...
class Trainer(object):
    def __init__(self, args, opts):
        ...
        self.class_loss_weight=self.opts['class_loss_weight']
    ...
    def resume(self, path):
        ...
        try :
            ...
            print('MCNN Model loading error ___ {}'.format(ex)) 
    ...
    def train(self, epoch):
        ...
        for step, data in enumerate(self.train_loader):
            ...
            try :
                ...
                try: 
                    ...
                    invalidSamples+=1
                    print('MCNN Model Input Exception : {}'.format(ex))
                    continue
                ...
                try:
                    ...
                    invalidSamples+=1
                    print('Classification Branch Issue : {}'.format(e))
                    continue
                ...
                loss_total = poly_loss + self.class_loss_weight*class_loss1

                # Accumulate 
                loss_sum = poly_loss+self.class_loss_weight*class_loss1
        ...
        # Wandb Tracking 
        wandb.log({'train_invalid_samples':invalidSamples,'epoch':self.epoch})       
        wandb.log({'train_avg_total_epoch_loss':avg_epoch_loss, 'epoch': self.epoch})
        wandb.log({'train_avg_poly_loss':avg_poly_loss1, 'epoch': self.epoch})
        wandb.log({'train_avg_classifier_loss':avg_class_losses,'epoch':self.epoch})

        print("Average Epoch %d loss is : %f and poly_loss is : %f & Class loss is : %f" % (epoch, avg_epoch_loss, avg_poly_loss1,class_loss1))
        return(weights)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for step, data in enumerate(tqdm(self.val_loader)):
                ...
                try : 
                    ...
                    w1 = torch.tensor(data["w"]).view(-1,1).cpu().float()
                    h1 = torch.tensor(data["h"]).view(-1,1).cpu().float()
                    ...
                    try:
                        ...
                        print('HD/HD951 Computation Error : {}'.format(exp))
        ...
        for key in final_ious:
            if key in list(set(testcount)):
                if int(testcount[key])==0:
                    ...
                    wandb.log({"{}_IoU_val".format(key):final_ious[key],"epoch":self.epoch})
            else:
                continue
        ...
        for key in final_acc:
            if key in list(set(testcount)):
                if int(testcount[key])==0:
                    ...
                    wandb.log({"{}_Acc_val".format(key):final_acc[key],"epoch":self.epoch})
            else:
                continue
        ...
        for key in final_hd:
            if key in list(set(testcount)):
                if int(testcount[key])==0:
                    ...
                    wandb.log({"{}_HD_val".format(key):final_hd[key],"epoch":self.epoch})
            else:
                continue
        ...
        for key in final_hd95:
            if key in list(set(testcount)):
                if int(testcount[key])==0:
                    ...
                    wandb.log({"{}_HD95_val".format(key):final_hd95[key],"epoch":self.epoch})
            else:
                continue     
        ...
        wandb.log({'val-valid-samples':len(losses),'epoch':self.epoch})
        wandb.log({'val-invalid-samples':countFalse,'epoch':self.epoch})
        ...
        wandb.log({'val-avg-class_loss-epoch':avg_class_loss,'epoch':self.epoch})
        wandb.log({'val-avg_acc-epoch':avg_acc,'epoch':self.epoch})

       
        # Other Metrics 
        wandb.log({'val-Net_IOU-epoch':100*np.mean(np.array(list(final_ious.values())).astype(np.float)),'epoch':self.epoch})
