...
class DataProvider(Dataset):
    ...
    def read_dataset(self):
        ...
        # data_list = glob.glob(osp.join(self.data_dir, nameFile))
    ...
    def prepare_component(self, instance, component):
        ...
        if(df1.find("http://10.5.0.142:9006/")>=0):
            n = 3
            start = df1.find("/")
        ...
        elif(df1.find('http://bhoomi.csa.iisc.ernet.in:8080/ihg/manuscript_editor/images/')>=0):
            splitList=df1.split('/')
            index=splitList.index('images')
            splitList=splitList[index:]
            df2='/'.join(splitList)
            i_url = "data/Bhoomi_data/"+df2

        elif(df1.find("static/imgdata/bhoomi/")>=0):
            splitList=df1.split('/')
            splitList=splitList[3:]
            df2='/'.join(splitList)
            i_url = "data/Bhoomi_data/images/"+df2
            
        
        elif(df1.find("ASR_Images")>=0 or df1.find("penn-in-hand")>=0 or df1.find("jain-mscripts")>=0 ):
            splitList=df1.split('/')
            ...
            # print('URL ISSUE : ',df1)
            print('Issue Instance Issue !',df1)
            emptyDict={}
            return emptyDict
        ...
        # try : 
        # print('URL : {} '.format(i_url))
        img = cv2.imread(i_url)
        w_image, h_image = img.shape[1], img.shape[0]
        img1 = imread(i_url)
        # except Exception as ex : 
        #     print('ImageLoader : {}'.format(ex))
        #     print('Image URL : {}'.format(i_url))
        #     # emptyDict={}
        #     # return emptyDict
        ...
        gt_label44 = np.zeros((9),np.float32)
        classes = ['Hole(Virtual)','Hole(Physical)','Character Line Segment', 'Character Component','Picture','Decorator','Library Marker', 'Boundary Line', 'Physical Degradation']
        for i in range(9):
            if label == classes[i]:
                gt_label = i
                gt_label44[i] = 1.0
